...
int BlockRecv::Routine()
{
	...
	while (1)
	{
		...
		{
			BlockRecvRun* pClient = new BlockRecvRun(s, addr);
...
BlockRecvRun::BlockRecvRun(SOCKET s, sockaddr_in addr)
{
	//初始化各成员变量。
	m_socket = s;
	m_addr = addr;
...
BlockRecvRun::~BlockRecvRun(void)
{
	delete[]m_pRecvData;
	delete[]m_pSendData;
}

bool BlockRecvRun::IsConnected()
{
	return m_IsConnected;
}

bool BlockRecvRun::set_senddata(std::string data)
{
	strcpy(m_pSendData, data.c_str());
	return true;
}

DWORD WINAPI BlockRecvRun::sendThread(void* param)//发送线程入口函数。
{
	//std::cout << "发送数据线程开始运行！！" << std::endl;
	BlockRecvRun* pClient = static_cast<BlockRecvRun*>(param);//获得BlockRecvRun对象指针。以便操纵成员变量。
...
DWORD WINAPI BlockRecvRun::recvThread(void* param)//接收数据线程入口函数。
{
	//std::cout << "接收数据线程开始运行！！" << std::endl;
	BlockRecvRun* pClient = static_cast<BlockRecvRun*>(param);
	while (pClient->m_IsConnected)
	{
...
bool BlockRecvRun::startRunning()
{
